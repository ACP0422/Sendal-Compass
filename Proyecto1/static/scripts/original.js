const money = (n) => {
  ...
  function getLotIdFromEl(el){
    // Prioriza data-lote, luego id
    const raw = el.getAttribute("data-lote") || el.id || "";
    return (raw || "").trim();
  }
  
  function applyLotState(el, estado){
    const st = (estado || "").toLowerCase().trim();
  
    el.classList.remove("lot-available", "lot-reserved", "lot-sold");
  
    // marca como lote "dibujable"
    el.classList.add("lot-shape");
  
    if (st === "vendido") {
      el.classList.add("lot-sold");
      el.dataset.estado = "vendido";
      return;
    }
  
    if (st === "apartado") {
      el.classList.add("lot-reserved");
      el.dataset.estado = "apartado";
      return;
    }
  
    // default: disponible
    el.classList.add("lot-available");
    el.dataset.estado = "disponible";
  }
  
  async function fetchLotState(lotId){
    const res = await fetch(`/api/lote/${encodeURIComponent(lotId)}/`, {
      headers: { "X-Requested-With": "XMLHttpRequest" },
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return null;
    return (data?.result?.estado_lote || "").toLowerCase().trim();
  }
  ...
  async function paintLotsFromDB(){
    const svg = document.querySelector("#mapWrap svg");
    if (!svg) return;
  
    // Ajusta el selector a como están tus lotes:
    // Si tus lotes son <path id="MZ04-L01"> o <polygon id="..."> etc.
    // Usa esto para agarrar figuras, no textos:
    const candidates = svg.querySelectorAll("path[id], polygon[id], rect[id]");
  
    const lotEls = [];
    for (const el of candidates){
      const lotId = getLotIdFromEl(el);
      // filtra solo ids que parecen lote
      // ajusta el patrón si tus ids cambian
      if (!/^MZ/i.test(lotId)) continue;
      lotEls.push(el);
    }
    ...
    await Promise.all(
      lotEls.map(async (el) => {
        const lotId = getLotIdFromEl(el);
        const estado = await fetchLotState(lotId);
        if (!estado) return;
        applyLotState(el, estado);
      })
    );
  }
// después de cargar el svg en #mapWrap...
paintLotsFromDB();
...
svg.addEventListener("click", async (e) => {
  const el = e.target.closest("path[id], polygon[id], rect[id]");
  if (!el) return;

  const lotId = getLotIdFromEl(el);
  if (!/^MZ/i.test(lotId)) return;

  // Si ya está pintado y no disponible, ni intentes abrir panel
  const cached = (el.dataset.estado || "").toLowerCase();
  if (cached && cached !== "disponible") {
    showToast("Este lote no está disponible.", false);
    return;
  }

  // Si todavía no sabemos el estado (primer click), lo consultamos
  if (!cached) {
    const estado = await fetchLotState(lotId);
    if (estado) applyLotState(el, estado);
  ...
  loadLot(lotId);
  openPanel(); // tu función si existe
});
...
function showLotTooltip(text, estado, x, y){
  if (!tooltip) return;

  tooltip.textContent = text;
  tooltip.className = `lot-tooltip ${estado}`;
...
mapWrap.addEventListener("mousemove", (e) => {
  ...
  const estado = (el.dataset.estado || "").toLowerCase();
  ...
  showLotTooltip(label, estado, e.pageX, e.pageY);
});
